...
var Frost = function(){
	...
	var handleCountDown = function(WebsiteLaunchDate){
		...
		if($(".countdown").length)
		{
			$('.countdown').countdown({date: WebsiteLaunchDate}, function() {
				$('#time-over').css({display: 'block'})
				$('#time-in').css({display: 'none'})
				$('.countdown').text('Переходи по ссылке ниже, для начала квеста');
			});
		}
	...
	var handleFinalCountDown = function(){
		...
		{
			var startTime = new Date( "Jan 3 2020 17:02:37" ); // Put your website start time here
